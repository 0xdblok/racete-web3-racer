...
Status: **Phase B.1 — read-only balance + automated safety checks**  
Last updated: 2026-06-21

## Current State

Token Stake Rooms are in **Phase B.1** — read-only test token balance display with automated safety guardrails.
...
- UI: `TokenStakeRoomsPreview` component showing disabled state + connected wallet test token balance on `/race/multiplayer`
...
- Token Stake Rooms feature flag: `TOKEN_STAKE_ROOMS_ENABLED=***
- Test mode active: `TOKEN_STAKE_ROOMS_TEST_MODE=***
- No deposits accepted (all deposit APIs return 403)
- No token transfers
- No token payouts
...
The script scans `<file>` (see below) and fails if any of these strings appear:
